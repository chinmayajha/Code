...
lli cnt, n, p, q, r;string s;
bool t_cases = 0;
bool usacoo = 0;
void solve(){
	cin>>s;
    // 3 vars, 1 counting W(first), one counting O,
    // if second W is found, add second O's count to ans
    rep(i,0,s.size()){
        if(s[i] == 'o') q+=p; //number of Os
        if(i>0 && s[i] == 'v' && s[i-1]=='v'){
            p++; //number of V
            r += q; //number of paired Vs ie W
        }
    }
    cout <<r;
}
