...
// void timee() {double time_taken = double(clock() - startTime) /
// double(CLOCKS_PER_SEC); cerr << "Time: " << time_taken << setprecision(5) <<
// " sec" << endl;}
void in(int &x) {
    bool neg = false;
    register int c;
...
vector<bool> prime(int(1e7 + 5), 1);
vector<int> prefix_prime(int(1e7 + 5));
void sieve() {
    prime[0] = prime[1] = false;
    int i, j;
    for (i = 2; i * i < int(1e7 + 5); i++) {
        if (prime[i]) {
            for (j = i * i; j <= int(1e7 + 5); j += i)
                prime[j] = false;
        }
    }
}
...
void solve() {
    ...
    if (n == 1) {
        ...
        cout << prefix_prime[n] - prefix_prime[(n / 2)] + 1 << "\n";
}
